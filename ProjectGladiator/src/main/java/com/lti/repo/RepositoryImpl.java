...
public class RepositoryImpl implements ProjectRepository {
	...
	public boolean findByEmail(String email) {
		return (long) em.createQuery(
				"select count(userDetails.userId) from UserDetails userDetails where userDetails.userEmail=:em ")
	...
	public int findByIdAndPassword(int userId, String password) {
		...
		int a = (int) em.createQuery("select u.userId from UserDetails u where u.userId=:id and u.userPassword=:pass")
				.setParameter("id", userId).setParameter("pass", password).getSingleResult();
	...
	public List<Object> getAllVehiclePoliciesByUserId(int userId) {
		String hql1 = "select vid.insurancePolicyId, vid.insuranceDuration, vid.insurancePremium, vid.insuranceStatus, vid.user.userId , vid.vehicledetails.registrationNumber, vid.vehicleinsuranceplan.insurancePlanId from VehicleInsuranceDetails vid where vid.user.userId=:uid";
		Query query1 = em.createQuery(hql1);
	...
	public List<Object> getAllTravelPoliciesByUserId(int userId) {
		String hql1 = "select tid.insurancePolicyId, tid.insuranceDuration, tid.insurancePremium, tid.insuranceStatus, tid.user.userId , tid.traveldetails.travelId, tid.travelinsuranceplan.insurancePlanId from TravelInsuranceDetails tid where tid.user.userId=:uid";
		Query query1 = em.createQuery(hql1);
		query1.setParameter("uid", userId);
		return query1.getResultList();
	}
	...
	public long checkVehicleClaimOnBasisOfStatus(int policyId) {
		String hql = "select COUNT(c.claimId) from ClaimDetails c where c.vehicleinsurancedetails.insurancePolicyId=:policy and c.status=:a";
		Query query1 = em.createQuery(hql);
		query1.setParameter("policy", policyId);
		query1.setParameter("a", "Pending");
	...
	@Override
	@Transactional
	public long chekTravelClaimOnBasisOfStatus(int policyId) {
		String hql1 = "select COUNT(c.claimId) from ClaimDetails c where c.travelinsurancedetails.insurancePolicyId=:policy and c.status=:a";
		Query query1 = em.createQuery(hql1);
		query1.setParameter("policy", policyId);
		query1.setParameter("a", "Pending");
		long num1 = (Long) query1.getSingleResult();
		return num1;
	}

	@Override
	public int findPolicyIdByUserId(int insurancePolicyId) {
		VehicleInsuranceDetails vid = em.find(VehicleInsuranceDetails.class, insurancePolicyId);
		UserDetails user = vid.getUser();
